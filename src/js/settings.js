...
exports.globalSettingsOpen = function() {
	...
	options.forEach((option, index) => {
		...
		optionInputEl.onchange = (event) => {
			...
			if (event.target.type == "range") {
				options[event.target.name].value = parseInt(event.target.value);
			}
		}
		...
		optionInputEl.oninput = (event) => {
			...
			options[event.target.name].value = val;
		};
	...
	// Check if any values are empty strings, if yes set them to the default value
